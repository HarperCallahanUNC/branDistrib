...
class TXModule(BRANDNode):
    def __init__(self):
        ...
        self.message_rate = self.parameters['message_rate']
    ...
    def send_udp(self):
        ...
        return sys.getsizeof(self.message)
    ...
    def run(self):
        ...
        while True:
            ...
            for s in range(0, self.fr_iterations):
                ...
                self.message_size = self.send_udp()
                # May want to refine but should send binned data at a rate we want 
                while time.monotonic() < self.last_time + self.message_size/self.message_rate: 
                    time.sleep(1e-6)
...
if __name__ == "__main__":
    ...
    # setup
    Spike_and_transmitter = TXModule()

    # main
    Spike_and_transmitter.run()
